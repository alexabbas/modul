...
oapl_ctdna_File <- paste0(
  ...
  "mo29112_ngs_dna_targdna_foundationoneliquidcdx_20-2047_n241_one-alteration-per-line-plus_20220125.txt"
)
liqOapl <- read.delim(oapl_ctdna_File,sep="\t") %>% select(-SUBJECT.ID)
ospl_ctdna_file = paste0(ctDnaDir,"mo29112_ngs_dna_targdna_foundationoneliquidcdx_20-2047_n241_one-sample-per-line_20220125.xlsx")
...
sampleMeta_file = paste0(ctDnaDir,"mo29112_F1L_sample_data_220217.xlsx")
...
oaplv1 <- oapl %>% filter(!grepl("BIOMARKER",VARIANT.TYPE))

# re-subset tissue and liq
tissueOapl <- oaplv1 %>% filter(Visit == "Tissue")
tissueOaplAll <- tissueOapl
tissueOapl <- tissueOapl %>% filter(SOMATIC.STATUS.FUNCTIONAL.IMPACT != "unknown")

liqOapl <- oaplv1 %>% filter(Visit != "Tissue")
liqOaplAll <- liqOapl
...
oaplv <- oaplv1 %>% filter(SOMATIC.STATUS.FUNCTIONAL.IMPACT != "unknown")
